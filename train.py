...
def get_args(parser):
    ...
    parser.add_argument("--bert_model", type=str, default="bert-base-chinese", choices=["bert-base-uncased", "bert-large-uncased", "bert-base-chinese"])
    ...
    parser.add_argument("--model", type=str, default="bow", choices=["bow", "img", "bert", "concatbow", "concatbert", "mmbt", "mmbt_drop"])
    ...
    parser.add_argument("--task", type=str, default="fakeddit", choices=["weibo", "vsnli", "food101", "fakeddit"])
...
class SCELoss(nn.Module):
    def __init__(self, alpha=0.8, beta=0.2, num_classes=2, device=torch.device('cuda')):
...
def model_eval(i_epoch, data, model, args, criterion, store_preds=False):
    with torch.no_grad():
        ...
        for batch in data:
            ...
            preds.append(pred)
            mypreds.extend(pred_x)
            tgt_num = tgt.cpu().detach().numpy()
            tgt_x = tgt.cpu().detach().tolist()
            tgts.append(tgt_num)
            mytgts.extend(tgt_x)

    metrics = {"loss": np.mean(losses)}
    metrics["f1"] = f1_score(mytgts,mypreds)
...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    if args.model == "bow":
        ...
        # if model.training:
        #     tgt = torch.cat([tgt,tgt], dim=-1).cuda()
        #     out1,_ = model(txt, mask, segment, img, use_r_layer=True)
        #     ce_loss = criterion(out1, tgt)
        #     return ce_loss, out1, tgt

        
        out1, fts1 = model(txt, mask, segment, img)
        tgt = tgt.cuda()
...
def train(args):
    ...
    for i_epoch in range(start_epoch, args.max_epochs):
        ...
        model.eval()
        metrics = model_eval(i_epoch, test_loaders, model, args, criterion)
