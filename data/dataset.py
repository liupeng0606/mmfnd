...
class JsonlDataset(Dataset):
    def __init__(self, data_path, tokenizer, transforms, vocab, args,is_test=False):
        ...
        self.text_start_token = ["[CLS]"] if args.model != "mmbt_drop" else ["[SEP]"]
    ...
    def __getitem__(self, index):
        if self.args.task == "vsnli":
            ...
            text = self.data[index]["cap"] + "," + self.data[index]["text"]
            sentence = (
                self.text_start_token
                + self.tokenizer(text)[: (self.args.max_seq_len - 1)
                ]
            )
        ...
        image = None
        if self.args.model in ["img", "concatbow", "concatbert", "mmbt", "mmbt_drop"]:
