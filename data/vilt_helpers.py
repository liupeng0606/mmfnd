...
from data.vilt_dataset import JsonlDataset
...
def get_vocab(args):
    vocab = Vocab()
    if args.model in ["bert", "mmbt", "mmbt_vilt", "mmbt_cap", "mmbt_cap_sigmoid", "concatbert"]:
...
def collate_fn(batch, args):
    ...
    img_tensor = None
    if args.model in ["img", "concatbow", "concatbert", "mmbt_cap", "mmbt", "mmbt_vilt"]:
    ...
    return text_tensor, segment_tensor, mask_tensor, img_tensor, tgt_tensor, [row[-2] for row in batch], [row[-1] for row in batch]


def get_data_loaders(args):
    tokenizer = (
        BertTokenizer.from_pretrained(args.bert_model, do_lower_case=True).tokenize
        if args.model in ["bert", "mmbt", "concatbert","mmbt_cap", "mmbt_vilt"]
        else str.split
    )
