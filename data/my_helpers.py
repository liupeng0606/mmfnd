...
from data.my_dataset import JsonlDataset
...
def get_labels_and_frequencies(path):
    label_freqs = Counter()
    data_labels = [json.loads(line)["2_way_label"] for line in open(path)]
...
def get_vocab(args):
    vocab = Vocab()
    if args.model in ["bert", "mmbt", "mmbt_cap", "mmbt_cap_sigmoid", "concatbert"]:
...
def collate_fn(batch, args):
    ...
    raw_text = list(map(lambda x: x[-1], batch))

    cap = list(map(lambda x: x[-2], batch))

    text_tensor

    img_tensor = None
    if args.model in ["img", "concatbow", "concatbert", "mmbt_cap", "mmbt"]:
    ...
    return text_tensor, segment_tensor, mask_tensor, img_tensor, tgt_tensor, cap, raw_text
