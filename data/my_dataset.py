...
class JsonlDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.args.task_type == "multilabel":
            ...
            label[
                [self.args.labels.index(tgt) for tgt in self.data[index]["2_way_label"]]
            ] = 1
        else:
            label = torch.LongTensor(
                [self.args.labels.index(self.data[index]["2_way_label"])]
            )

        image = None
        if self.args.model in ["img", "concatbow", "concatbert", "mmbt", "mmbt_cap"]:
