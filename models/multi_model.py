...
class MultimodalBertEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        bert = BertModel.from_pretrained(args.bert_model, num_hidden_layers=12)
...
class DROPMultimodalBertClf(nn.Module):
    def __init__(self, args):
        super(DROPMultimodalBertClf, self).__init__()
        self.args = args
        self.enc = MultimodalBertEncoder(args)
        self.clf = nn.Linear(args.hidden_sz, args.n_classes)
        self.r_layer = TransformerEncoderLayer(768,12,768,0.2)
    ...
    def forward(self, txt, mask, segment, img, use_r_layer=False):

        x = self.enc(txt, mask, segment, img)

        # if self.training:
        #     if use_r_layer:
        #         r_x1 = self.r_layer(x.unsqueeze(1)).squeeze(1)
        #         fts1 = self.r_cls(r_x1)
        #         r_x1 = torch.cat([x, fts1], dim=0)
        #         return self.clf(r_x1), fts1
