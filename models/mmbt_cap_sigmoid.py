...
class MY_MultimodalBertClf_sigmoid(nn.Module):
    def __init__(self, args):
        super(MY_MultimodalBertClf_sigmoid, self).__init__()
        self.args = args
        self.enc = MultimodalBertEncoder(args)
        self.clf = nn.Linear(args.hidden_sz, args.n_classes)
        self.tokenizer = BertTokenizer.from_pretrained(args.sentiment_model)
        self.sentiment_bert = BERTMODEL.from_pretrained(args.sentiment_model)
        self.dynamic = nn.Sequential(
            nn.Linear(args.hidden_sz * 2, args.hidden_sz),
            nn.ReLU(),
            nn.Linear(args.hidden_sz, 1),
            nn.Sigmoid())


    def forward(self, txt, mask, segment, img, raw_text=None, cap=None):
        x = self.enc(txt, mask, segment, img)



        encoding = self.tokenizer(cap, raw_text, padding='longest', truncation=True, max_length=90, return_tensors="pt")
        encoding["input_ids"] = encoding["input_ids"].cuda()
        encoding["token_type_ids"] = encoding["token_type_ids"].cuda()
        encoding["attention_mask"] = encoding["attention_mask"].cuda()
        bert_output = self.sentiment_bert(**encoding).last_hidden_state[:,0,:]

        concat_x = torch.cat([x, bert_output], dim=-1)

        a = self.dynamic(concat_x)

        out = a * x + ( 1 - a ) * bert_output
